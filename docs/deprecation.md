...
- **Establish a clear taxonomy of data types, and be careful of how this interacts with code base scope.** There are many kinds of things that can show up in an input data frame for the supergroups. You have to know what they are and establish rules for how the code handles them and what is allowed. Some examples:
  - Variables we want to impute: rates and counts of, for example, vaccinated individuals in each state, to be imputed in counties. These should only be touched in the final imputing step.
  - Variables that _define the groupings_. For example, the state and county. These should only be touched in the initial mapping step.
  - Depending how the imputer is scoped, additional variables may be lying around which will be used for down-stream further stratification, and which therefore ought to be retained in subgroup data. Or one could disallow any extraneous data.
